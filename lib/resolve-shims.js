...
var resolve = module.exports = function resolveShims (file, messages, cb) {
  var shimConfig = arguments[3] || null;
  // find the package.json that defines browserify-shim config for this file
  mothership(file, function (pack) { return !! shimConfig || !! pack['browserify-shim'] }, function (err, res) {
    ...
    var cacheKey = packageDir;
    if (shimConfig) {
      cacheKey += JSON.stringify(shimConfig);
    }

    // we cached this before which means it was also grouped by file
    var cached = shimsCache[cacheKey];
    ...
    if (cached) { 
      return cb(null, { 
        ...
        , 'browserify-shim'  :  shimConfig || pack['browserify-shim']
      ...
      try {
        ...
        var shimField = shimConfig || pack['browserify-shim'];
        ...
        updateCache(cacheKey, pack, resolved.shims, resolved.exposeGlobals);
